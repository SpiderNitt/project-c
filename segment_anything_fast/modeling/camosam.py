...
class CamoSam(L.LightningModule):
    ...
    def log_images(self, information, img_dict, sep_mask_dict, mask_dict, gt_mask_dict, iou_dict, batch_idx, train=True):
        ...
        for frame_idx in range(len(information)):
            ...
            self.logger.log_image(f"Images/{'train' if train else 'val'}/{information[frame_idx]['name']}", [img_dict[t][frame_idx] for t in range(start_idx, self.cfg.dataset.num_frames)], step=self.global_step, masks=log_dict,
    ...
    def bootstrapped_ce(self, inputs, targets, it):
        inputs = inputs.flatten(-2)	 # [num_true_obj, C, HxW]
        targets = targets.flatten(-2) # [num_true_obj, C, HxW]
        if it < self.cfg.start_warm:
            return F.binary_cross_entropy_with_logits(inputs, targets, reduction='none').mean(dim=-1)

        raw_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
        num_pixels = raw_loss.numel()

        if it > self.cfg.end_warm:
            this_p = self.cfg.top_p
        else:
            this_p = self.cfg.top_p + (1-self.cfg.top_p)*((self.cfg.end_warm-it)/(self.cfg.end_warm-self.cfg.start_warm))
        loss, _ = torch.topk(raw_loss, int(num_pixels * this_p), sorted=False, dim=-1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for t in range(start_idx, self.cfg.dataset.num_frames):
            ...
            for each_output, gt_mask, dense_embed, selector, resize_longest_size in zip(outputs, batch['gt_mask_256'], prop_log_dict["prop_dense_embed"], batch['selector'], batch['resize_longest_size']): # selector = [True, True, False]
                ...
                loss_bce_tmp = self.bootstrapped_ce(pred_masks, gt_mask, self.global_step)
                ...
                loss_bce += loss_bce_tmp[batch_indexing, min_idx][selector].sum()
        ...
        avg_bce = (self.cfg.bce_wt * loss_bce) / (total_num_objects)
        ...
        log_dict = {"Loss/train/total_loss" : loss_total, "Loss/train/focal_loss" : avg_focal, "Loss/train/bce_loss" : avg_bce, "Loss/train/dice_loss" : avg_dice, "Loss/train/iou_loss" : avg_iou, "Loss/train/dense_reg": avg_dense_regularization}
    ...
    def validation_step(self, batch, batch_idx):
        if self.cfg.dataset.stage1:
            ...
            self.log_dict({"Loss/val/total_loss" : loss_total, "Loss/val/focal_loss" : avg_focal, "Loss/val/bce_loss" : avg_bce, "Loss/val/dice_loss" : avg_dice, "Loss/val/iou_loss" : avg_iou, "Loss/val/dense_reg": avg_dense_regularization}, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=bs)
        ...
        if self.cfg.dataset.stage1:
            ...
            for t in range(start_idx, self.cfg.dataset.num_frames):
                ...
                for each_output, gt_mask, cropped_img, selector in zip(output_, batch['gt_mask'], batch['cropped_img'], batch['selector']):
                    ...
                    img_dict[t].append(cropped_img[t])
                metrics_all[f'Metrics/val/jaccard_single_obj_{t}'] = total_jaccard / total_objects
                metrics_all[f'Metrics/val/dice_single_obj_{t}'] = total_dice / total_objects
