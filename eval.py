...
def metrics_eval(pred_mask_list, vid):
    sm_total, wf_total, e_total = 0, 0, 0
    frames_count = len(pred_mask_list)
    assert len(pred_mask_list) == len(os.listdir(f'raw/MoCA-Mask/MoCA_Video/TestDataset_per_sq/{vid}/GT'))
    for i, frame in enumerate(sorted(os.listdir(f'raw/MoCA-Mask/MoCA_Video/TestDataset_per_sq/{vid}/GT'))):
        gt = np.asarray(Image.open(f'raw/MoCA-Mask/MoCA_Video/TestDataset_per_sq/{vid}/GT/{frame}'), dtype=np.int32) // 255
        pred = pred_mask_list[i]
        sm = StructureMeasure(pred, gt)
        wf = original_WFb(pred, gt)
        e = EnhancedMeasure(pred, gt)
        sm_total += sm
        wf_total += wf
        e_total += e
    metrics_all = {'Metrics/sm': sm_total / frames_count, 'Metrics/wf': wf_total / frames_count, 'Metrics/e': e_total / frames_count}
    return metrics_all