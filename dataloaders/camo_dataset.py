...
class VideoDataset(data.Dataset):
    def __init__(
        ...
        print(f'{"Train" if self.train else "Val"} Loaded COD10K', count)
        assert len(self.image_list) == len(self.gt_list)
          
        
        print(f'===={"Train" if self.train else "Val"} Loaded {count} images')
...
def get_loader(cfg):
    ...
    val_dataset = VideoDataset(cfg, train=False)
