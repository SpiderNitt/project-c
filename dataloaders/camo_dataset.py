...
class VideoDataset(data.Dataset):
    def __init__(
        ...
        count = len(self.image_list)
        print(f'{"Train" if finetune else "Test"} Loaded CAAD10K', len(self.image_list) - count)
        count = len(self.image_list)
        assert len(self.image_list) == len(self.gt_list)
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pad
        h, w = x.shape[-2:]
        padh = self.trainsize - h
        padw = self.trainsize - w
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(all_gt_copy)):
            all_gt_copy[i] = self.gt_transform(all_gt_copy[i])[0]
        
            all_gt[i] = Image.fromarray(self.preprocess(torch.as_tensor(self.transform.apply_image(np.asarray(all_gt[i], dtype=np.uint8)))
            ).numpy().astype(np.uint8)*255).resize((256,256))
        ...
        for gt_mask in all_gt:     #### for each object/instance in the image   
...
def get_loader(
    ...
    train_dataset = VideoDataset(dataset, seq_len, trainsize, iou_range=iou_range, max_prompt_points=max_prompt_points, finetune=True, is_validation_prompt_mid = False)
    ...
    val_dataset = VideoDataset(dataset, seq_len, trainsize, iou_range=iou_range, max_prompt_points=max_prompt_points, finetune=False, is_validation_prompt_mid = True)
